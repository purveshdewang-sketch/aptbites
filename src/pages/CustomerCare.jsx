      ...
      <div className="mx-auto max-w-md">
        ...
        <section className={`mt-5 overflow-hidden ${CARD}`}>
          <div className="bg-[#073B35] p-5 text-white">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm font-semibold leading-relaxed text-white/70">
                  Our support agent can help with order details, payment status,
                  packing choice, and issue type.
                </p>
              </div>
            </div>
            ...
            <Link
              to="/care-agent"
              className="mt-5 block rounded-2xl border border-[#41D3BD] bg-[#41D3BD] py-4 text-center font-black text-[#073B35] active:scale-[0.98]"
            >
              Start Chat
            </Link>
          </div>
          ...
          <div className="mt-5 space-y-3">
            {quickActions.map((item) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-black text-[#073B35]">{item.title}</p>
                ...
                <span className="shrink-0 text-lg font-black text-[#0B8F80]">
                  →
                </span>
              </Link>
            ))}
        ...
        <section className={`mt-5 p-5 ${CARD}`}>
          ...
          <div className="mt-5 grid grid-cols-1 gap-3">
            ...
            <a
              href={`mailto:${SUPPORT_EMAIL}`}
              className="rounded-2xl border border-[#41D3BD] bg-[#41D3BD] px-4 py-4 text-center font-black text-[#073B35] active:scale-95 break-all"
            >
              {SUPPORT_EMAIL}
            </a>
          </div>

          <p className="mt-5 text-xs leading-relaxed text-[#8AA5A0]">
        ...
        <section className={`mt-5 p-5 ${CARD}`}>
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#BDEFE6] bg-[#41D3BD]/12 text-2xl">
            ✉️
          </div>

          <h2 className="mt-4 text-2xl font-black text-[#073B35]">
            Need email help?
          </h2>
          ...
          <a
            href={`mailto:${SUPPORT_EMAIL}`}
            className="mt-4 inline-block break-all font-black text-[#0B8F80]"
          >
            {SUPPORT_EMAIL}
          </a>
        </section>
...
function InfoTile({ icon, title, text }) {
  return (
    <div className="rounded-[22px] border border-[#D7F5EF] bg-white/90 p-3 text-center shadow-[5px_5px_14px_rgba(7,59,53,0.06),-5px_-5px_14px_rgba(255,255,255,0.95)]">
      <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-2xl border border-[#BDEFE6] bg-[#41D3BD]/12 text-xl">
        {icon}
      </div>

      <p className="mt-2 text-sm font-black text-[#073B35]">{title}</p>
