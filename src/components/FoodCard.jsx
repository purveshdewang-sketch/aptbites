...
const CARD =
  "rounded-[24px] border border-[#EADFCE] bg-white/95 shadow-[7px_7px_18px_rgba(63,81,40,0.07),-7px_-7px_18px_rgba(255,255,255,0.95)]";
...
function saveFavorites(favorites) {
  ...
  window.dispatchEvent(
    new CustomEvent("Nefo_favorites_updated")
  );
}
...
export default function FoodCard({ item }) {
  ...
  const cartItem = cartItems.find(
    (currentCartItem) =>
      String(currentCartItem.id) === String(item.id)
  );
  ...
  function handleToggleFavorite(event) {
    ...
    const nextFavorites = [
      ...
      ...currentFavorites.filter(
        (currentFavoriteItem) =>
          getFavoriteId(currentFavoriteItem) !== itemId
      ),
    ];
  ...
  function MobileCartButton() {
    if (quantity === 0 || isBlocked) {
      return (
        <button
          type="button"
          onClick={handleAddToCart}
          disabled={isBlocked}
          className={`h-10 min-w-[94px] rounded-[15px] border px-4 text-xs font-black transition-all ${
            isBlocked
              ? "cursor-not-allowed border-red-100 bg-[#F1E8DC] text-[#9A8E80]"
              : "border-[#3F5128] bg-[#3F5128] text-white shadow-md shadow-[#3F5128]/15 active:scale-95"
          }`}
        >
          {getButtonLabel()}
        </button>
    ...
    return (
      <div className="flex h-10 items-center overflow-hidden rounded-[15px] border border-[#3F5128] bg-[#3F5128] font-black text-white shadow-md shadow-[#3F5128]/15">
        <button
          type="button"
          onClick={handleDecrease}
          className="flex h-full w-9 items-center justify-center text-lg active:bg-[#4D612F]"
          aria-label="Decrease quantity"
        >
          −
        </button>

        <span className="flex h-full min-w-9 items-center justify-center bg-[#CF743D] px-2 text-xs text-white">
          {quantity}
        </span>
      ...
      >
        ...
        <div className="p-3 sm:hidden">
          <div className="flex items-stretch gap-3">
            <div className="relative min-h-[144px] w-[112px] shrink-0 overflow-hidden rounded-[20px] border border-[#D8C9B3] bg-[#FFF0DF]">
              <img
                src={imageSrc}
                alt={item.name}
              ...
              <button
                ...
                className={`absolute right-2 top-2 z-30 flex h-8 w-8 items-center justify-center rounded-full border text-base shadow-md active:scale-95 ${
                  favorite
              ...
              !isSoldOut &&
              !fulfillmentUnavailable ? (
                <div className="absolute bottom-2 left-2 right-2 z-20 rounded-xl border border-[#EADFCE] bg-white/95 px-2 py-1 backdrop-blur">
              ...
              {isBlocked ? (
                <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-black/60 px-2 text-center">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <h3
                      className={`line-clamp-2 text-[15px] font-black leading-[1.2] ${
                        kitchenIsClosed ||
                        fulfillmentUnavailable
                          ? "text-[#9A8E80]"
                          : "text-[#181411]"
              ...
              <div className="mt-2 flex min-w-0 items-center gap-1.5 text-[10px]">
                {ratingCount > 0 ? (
                  <>
                    ...
                    <span className="font-black text-[#3F5128]">
                      {ratingAverage.toFixed(1)}
                    </span>

                    <span className="text-[#9A8E80]">
                      ({ratingCount})
                    </span>

                    <span className="text-[#D8C9B3]">
                      •
                    </span>
                  </>
                ) : null}

                <p className="min-w-0 truncate text-[#6B6258]">
                  Ready{" "}
              ...
              <div className="mt-1 flex items-center gap-1">
                ...
                <p
                  className={`text-[10px] font-black ${getAvailabilityClass()}`}
                >
                  {getAvailabilityText()}
                </p>
              </div>
              ...
              {item.description ? (
                ...
                >
                  <p className="line-clamp-1 text-[10px] leading-relaxed text-[#6B6258]">
                    {item.description}
                  </p>
                </Link>
              ) : null}
